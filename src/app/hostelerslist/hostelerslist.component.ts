...
@Component({
  selector: 'app-hostelerslist',
  templateUrl: './hostelerslist.component.html',
  styleUrls: ['./hostelerslist.component.css']
})
export class HostelerslistComponent implements OnInit {
  hid='all';
  ...
  ngOnInit() {
    ...
    this._apiService.page = 'hostelers';
    ...
    this._apiService.getRoomType().subscribe(list => {
      ...
      this._apiService.getRoomsList(val).subscribe(lists => {
        ...
        const vals = { roomtype: 'all', hid: this.hostelsdata[0].hid,roomno:'all' }
        this.topForm.patchValue({
          roomtype: 'all',
          hid: this.hostelsdata[0].hid,
          roomno:'all'
    
        });
    
          this._apiService.getOccupancyData(vals).subscribe(list => {
            console.log(list);
            this.roomsdata = list.data;
  ...
  submit(value) {
    ...
    this._apiService.getOccupancyData(value).subscribe(list => {
      console.log(list);
      this.roomsdata = list.data;
  ...
  getbedss(event) {
    ...
    const val = {
      hostelid: this.fullview['hostelid'],
      roomno: this.fullview['rcid']
    }
